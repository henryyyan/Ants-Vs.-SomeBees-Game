...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while p.is_hive == False:
            if p.bees and self.lower_bound <= i and i <= self.upper_bound:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew != 0:
            ...
            if self.place.bees:
                Insect.reduce_health(b, b.health)
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        # BEGIN Problem 8a
        BodyguardAnt.action(self, gamestate)
        b = self.place.bees[:]
        for B in b:
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if gamestate.queen_bee == 0:
            gamestate.queen_bee = 1
...
def effect_bee(effect, bee, time):
    act = bee.action
    act2 = effect(bee.action, bee)
